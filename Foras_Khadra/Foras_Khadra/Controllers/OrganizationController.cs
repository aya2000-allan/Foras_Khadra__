...
namespace Foras_Khadra.Controllers
{
    public class OrganizationController : Controller
    {
        ...
        public async Task<IActionResult> ProfileSetting(OrganizationProfileViewModel model)
        {
            ...
            // تحديث جميع الحقول المطلوبة بطريقة آمنة
            organization.Name = string.IsNullOrWhiteSpace(model.Name) ? organization.Name : model.Name;
            organization.Sector = string.IsNullOrWhiteSpace(model.Sector) ? organization.Sector : model.Sector;
            organization.Country = string.IsNullOrWhiteSpace(model.Country) ? organization.Country : model.Country;
            organization.Location = model.Location ?? organization.Location;
            organization.ContactName = string.IsNullOrWhiteSpace(model.ContactName) ? organization.ContactName : model.ContactName;
            organization.PhoneNumber = string.IsNullOrWhiteSpace(model.PhoneNumber) ? organization.PhoneNumber : model.PhoneNumber;
            organization.Website = string.IsNullOrWhiteSpace(model.Website) ? organization.Website : model.Website;

            // لا تلمس organization.Password هنا!
